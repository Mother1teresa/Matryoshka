...
const waterType = ref(route.query.waterType || 'yachts');
...
const currentSubLinks = computed(() => {
  ...
  if (section) {if (!section.title || section.title.trim() === "") {const activeLink = section.links?.find(l => l.slug === sectionParam.value);return activeLink?.subLinks || [];}return section.links || [];}return [];});
// 2. ОПЦИИ
...
function applyFilters() {
  ...
  const cleanQuery = Object.fromEntries(
    Object.entries({ 
      ...
      ...filters }).filter(([key, v]) => 
      v !== '' && v !== null && v !== undefined && key !== 'subcategory'));
  router.push({
    name: 'catalog',
    params: { 
      ...route.params, 
      subcategory: subParam},query: cleanQuery});}
  ...
  if (newVal !== oldVal) {
    ...
    router.replace({
      ...
      params: { 
        ...route.params, 
        section: newVal,
        subcategory: targetSub || undefined },query: {} });}});

const currentConfig = computed(() => {
  let config = null;

  if (typeParam.value === 'transport' && sectionParam.value === 'water') {
    config = productFields.transport.water?.[waterType.value];
  } else if (typeParam.value === 'nedvizhimost') {
    config = productFields.nedvizhimost?.default;
  } else if (typeParam.value === 'transport') {
    config = productFields.transport?.[sectionParam.value];
  } else {
    config = productFields[typeParam.value]?.default || productFields[typeParam.value];
  }
  return config || { main: [], extra: [] };
});

const mainFields = computed(() => {
  return (currentConfig.value.main || []).map(updateLabels);
});
const extraFields = computed(() => {
  return (currentConfig.value.extra || []).map(updateLabels);
});
function updateLabels(field) {
  if (field.key === 'priceFrom') {
    field.label = dealType.value === 'rent' ? 'Аренда от' : 'Цена от';
  }
  if (field.key === 'priceTo') {
    field.label = dealType.value === 'rent' ? 'Аренда до' : 'Цена до';
  }
  return field;
}
    ...
    <div class="filter-card">
      <div class="filter-header">
        ...
        <div class="filter-main-row">
            ...
            <div v-if="isExpanded && hasExtra" class="filter-extra">
              <div class="transport-grid-container">
                <template v-for="field in extraFields" :key="field.key">
                  <div class="extra-group">
                    <label class="chips-label">{{ field.titlelabel }}</label>
                    <!-- SELECT -->
                    <multiselect
                    ...
                    <!-- CHIPS -->
                    <div v-else-if="field.type === 'chips'" class="chips-block">
                      <label class="chips-label">{{ field.label }}</label>
                      ...
                      <div class="chips">
                        <button
                          ...
                          :key="v"
                          class="chip"
                          :class="{ active: form[field.key] === v }"
                          @click="form[field.key] = v"
                        >
                          {{ v }}
                        </button>
                      </div>
...
.extra-group label { display: block; font-size: 1.25rem; }
...
.filter-grid{
  ...
  justify-content: space-between;
    ...
    <!-- <div v-if="type === 'realty'" class="deal-selector">
      <template v-if="sectionParam === 'houses'">
        <button v-for="sub in houseSubcategories" :key="sub.slug" class="deal-btn" 
          :class="{ active: $route.query.subcategory === sub.slug }" @click="selectSubcategory(sub.slug)">
          {{ sub.name }}
        </button>
      </template>
      <template v-else-if="sectionParam === 'commercial'">
        <button v-for="sub in commSubcategories" :key="sub.slug" class="deal-btn" 
          :class="{ active: $route.query.subcategory === sub.slug }" @click="selectSubcategory(sub.slug)">
          {{ sub.name }}
        </button>
      </template>
      <template v-else-if="sectionParam === 'parking'">
        <button class="deal-btn" :class="{ active: $route.query.subcategory === 'garage' }" @click="selectSubcategory('garage')">Гараж</button>
        <button class="deal-btn" :class="{ active: $route.query.subcategory === 'parking_place' }" @click="selectSubcategory('parking_place')">Паркинг</button>
      </template>
      <template v-else>
        <button class="deal-btn" :class="{ active: $route.query.subcategory === 'buy' }" @click="dealType = 'buy';">Купить</button>
        <button class="deal-btn" :class="{ active: $route.query.subcategory === 'rent' }" @click="dealType = 'rent';">Снять</button>
      </template>
    </div> -->